...
class Website(BaseModel):
    ...
    extra_domain = models.CharField(
        ...
        null=True,
        blank=True,
        verbose_name="additional domain name",
    ...
    ssl_config = models.JSONField(
        default=WebsiteMethod.get_ssl_config_default,
        blank=True,
        null=True,
        verbose_name="证书配置",
    )
    ssl_enable = models.BooleanField(default=False, verbose_name="enable SSL")
    index_root = models.CharField(
        default="/var/www/html", max_length=4096, verbose_name="站点目录"
    )
    status = models.IntegerField(
        choices=StatusType.choices,
        default=StatusType.READY,
        verbose_name="状态",
        help_text=StatusType.labels,
    )
    status_info = models.CharField(
        default="", blank=True, null=True, max_length=2048, verbose_name="status info"
    )

    web_server_type = models.IntegerField(
        default=WebServerType.Nginx, choices=WebServerType.choices
    )
    application = models.CharField(max_length=64, verbose_name="application")
    application_config = models.JSONField(
        default=dict, verbose_name="application config"
    )

    valid_web_server_config = models.TextField(
        max_length=102400,
        default=None,
        blank=True,
        null=True,
