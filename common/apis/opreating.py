...
from common.serializers.operating import (ExecuteCommandSyncSerializer,
...
class OperatingView(GenericViewSet):
    ...
    @extend_schema(responses=OperatingResSerializer, parameters=[
        OpenApiParameter(name="event_id", type=str, location=OpenApiParameter.QUERY,
                         description="event id", required=True)
    ])
    @action(methods=['get'], detail=False, serializer_class=QueryOperatingResSerializer,
            permission_classes=[permissions.IsAuthenticated])
    def query(self, request):
        event_id = request.query_params.get("event_id")
        data = BaseOperatingRes.get_instance(event_id)
        if data is None:
            data = BaseOperatingRes(
                event_id=event_id)
