...
def add_user(username, password, scope, data: dict = None):
    data["users"].append({"username": username, "password": password, "scope": scope})


@sync(WEBDAV_CONFIG_YAML)
def del_user(username, password, scope, data: dict = None):
    data["users"].remove({"username": username, "password": password, "scope": scope})
...
def download(version="v4.2.1"):
    ...
    os.system(
        "touch /run/ui-ssh-webdav.socket && chown www-data.www-data /run/ui-ssh-webdav.socket"
    )
    os.system(
        f"cp {PROJECT_ROOT}/backend/webdav/utils/config.yaml {PROJECT_ROOT}/webdav/config.yaml"
    )
...
def install_service():
    ...
    os.system(
        f"cp {PROJECT_ROOT}/backend/webdav/utils/webdav_server.sh {PROJECT_ROOT}/webdav/webdav_server.sh"
    )
    os.system(f"chmod a+x {PROJECT_ROOT}/webdav/webdav_server.sh")
    os.system(
        f"cp  {PROJECT_ROOT}/backend/webdav/utils/webdav.service.example {webdav_server_lib}"
    )
...
def update_config():
    ...
    if "location /webdav" not in default_config_data:
        default_config_data = append_server_nginx_config(
            default_config_data, webdav_config_data
        )

    if "location /webdav" not in backend_ssl_data:
        backend_ssl_data = append_server_nginx_config(
            backend_ssl_data, webdav_config_data
        )
